...
-- Superuser-only guard for sensitive platform-wide reporting.
...
-- Business-scoped, de-identified prevalence metrics based on the latest
-- non-test emergency declaration snapshot per worker within the selected window.
create or replace function public.get_business_deidentified_condition_prevalence(
  p_business_id text,
...
revoke all on function public.get_business_deidentified_condition_prevalence(text, timestamptz, timestamptz, integer) from public;

grant execute on function public.assert_current_user_is_superuser() to authenticated;
grant execute on function public.get_business_deidentified_condition_prevalence(text, timestamptz, timestamptz, integer) to authenticated;

comment on function public.get_business_deidentified_condition_prevalence(text, timestamptz, timestamptz, integer) is
  'Superuser-only, business-scoped, de-identified worker condition prevalence metrics using latest non-test emergency declaration snapshot per worker. Suppresses metrics when cohort size is below threshold.';

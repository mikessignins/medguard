...
export async function PATCH(
  ...
  try {
    ...
    try {
      ...
      const { error: serviceError } = await service.rpc('review_emergency_submission_authorized', {
      ...
      if (serviceError) {
        console.warn('[review/PATCH] authorized RPC failed; falling back to authenticated RPC', {
          submissionId: parsedId.value,
          actorUserId: userId,
          message: serviceError.message,
        })

        const { error: authError } = await authClient.rpc('review_emergency_submission', {
          p_submission_id: parsedId.value,
          p_status: status,
          p_note: note ?? null,
          p_expected_version: current.version,
      ...
      console.warn('[review/PATCH] authorized RPC path threw; falling back to authenticated RPC', {
        submissionId: parsedId.value,
        actorUserId: userId,

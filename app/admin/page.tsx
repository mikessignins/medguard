...
export default async function AdminPage() {
  ...
  const metrics = [
    { title: 'Workers', value: workerCount ?? 0, color: 'text-blue-400' },
    { title: 'Active Medics', value: medicCount ?? 0, color: 'text-emerald-400' },
    { title: 'Pending Medics', value: pendingCount ?? 0, color: 'text-amber-400' },
    { title: 'Sites', value: siteCount ?? 0, color: 'text-violet-400' },
    { title: 'Declarations This Month', value: submissionsThisMonth ?? 0, color: 'text-cyan-400' },
  ]
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {metrics.map(m => <MetricCard key={m.title} title={m.title} value={m.value} color={m.color} />)}
      </div>
    </div>
  )
}

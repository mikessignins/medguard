...
type SignupRole = 'medic' | 'occ_health'
...
const SUCCESS_MESSAGE: Record<SignupRole, string> = {
...
export default function StaffSignupForm({ defaultRole = 'medic' }: { defaultRole?: SignupRole }) {
  ...
  const submitLabel = useMemo(() => {
    return role === 'occ_health' ? 'Sign up for occ health' : 'Sign up as medic'
  }, [role])
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!response.ok) {
        setError(payload.error || 'Unable to request staff access.')
        return
      }

      setSuccess(payload.message || SUCCESS_MESSAGE[role])
      setDisplayName('')
      setEmail('')
      setPassword('')
      setConfirmPassword('')
    ...
    <div className="min-h-screen bg-slate-950 px-4 py-10 text-slate-100">
      <div className="mx-auto w-full max-w-md">
        <div className="mb-8 text-center">
          <h1 className="text-2xl font-bold tracking-tight">Staff sign-up</h1>
          <p className="mt-2 text-sm text-slate-400">
            Use the invite code from the business admin, choose your role, then sign in with your own password while approval is pending.
          </p>
        </div>
        ...
        <form onSubmit={submit} className="space-y-4 rounded-xl border border-slate-700/50 bg-slate-900/70 p-6">
          ...
          <div>
            ...
            <select
              ...
              className="w-full rounded-lg border border-slate-700 bg-slate-950/70 px-4 py-2.5 text-sm text-slate-100 outline-none transition-colors focus:border-cyan-500 focus:ring-2 focus:ring-cyan-500/40"
            >
              <option value="medic">{ROLE_LABEL.medic}</option>
              <option value="occ_health">{ROLE_LABEL.occ_health}</option>
            </select>
          </div>
